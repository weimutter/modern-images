...
const CategoriesMixin = {
  ...
  async getImagesByCategory(categoryId) {
    try {
      const client = await this.pool.connect();
      try {
        const sql = 'SELECT * FROM images WHERE category_id = $1 ORDER BY upload_time DESC, CASE WHEN SUBSTRING(filename FROM 17 FOR 3) ~ \'^[0-9]+$\' THEN CAST(SUBSTRING(filename FROM 17 FOR 3) AS INTEGER) ELSE 0 END ASC';
        const result = await client.query(sql, [categoryId]);
  ...
  async getImagesByCategoryPaged(categoryId, page = 1, limit = 50) {
    try {
      ...
      try {
        ...
        const dataSql = 'SELECT * FROM images WHERE category_id = $1 ORDER BY upload_time DESC, CASE WHEN SUBSTRING(filename FROM 17 FOR 3) ~ \'^[0-9]+$\' THEN CAST(SUBSTRING(filename FROM 17 FOR 3) AS INTEGER) ELSE 0 END ASC LIMIT $2 OFFSET $3';
  ...
  async getImagesWithoutCategoryPaged(page = 1, limit = 50) {
    try {
      ...
      try {
        ...
        const offset = (page - 1) * limit;
        const dataSql = 'SELECT * FROM images WHERE category_id IS NULL ORDER BY upload_time DESC, CASE WHEN SUBSTRING(filename FROM 17 FOR 3) ~ \'^[0-9]+$\' THEN CAST(SUBSTRING(filename FROM 17 FOR 3) AS INTEGER) ELSE 0 END ASC LIMIT $1 OFFSET $2';
  ...
  async getImagesWithoutCategory() {
    try {
      const client = await this.pool.connect();
      try {
        const sql = 'SELECT * FROM images WHERE category_id IS NULL ORDER BY upload_time DESC, CASE WHEN SUBSTRING(filename FROM 17 FOR 3) ~ \'^[0-9]+$\' THEN CAST(SUBSTRING(filename FROM 17 FOR 3) AS INTEGER) ELSE 0 END ASC';
        const result = await client.query(sql);
        return result.rows.map(row => ({
          filename: row.filename,
          path: row.path,
      ...
      try {
        ...
        if (subCategoryIds.length === 0) {
          sql = `
            SELECT * FROM images
            WHERE category_id = $1
            ORDER BY upload_time DESC, CASE WHEN SUBSTRING(filename FROM 17 FOR 3) ~ '^[0-9]+$' THEN CAST(SUBSTRING(filename FROM 17 FOR 3) AS INTEGER) ELSE 0 END ASC
            LIMIT $2 OFFSET $3
          `;
